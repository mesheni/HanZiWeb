...
async function main() {
  ...
  // поэтому `origin: '*'` недопустим. Браузерные запросы пропускаются
  // только с Origin из списка `CORS_ORIGIN`; запросы без Origin
  // (mobile SDK, curl) — не CORS-сценарий, пропускаются.
