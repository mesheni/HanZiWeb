...
function stabilityAfterSuccess(rating: SrsRating, stability: number, difficulty: number, r: number): number {
  const s = Math.max(0.01, stability);
  const sInc = 1 + Math.exp(W[8]) * (11 - difficulty) * Math.pow(s, DECAY) * (Math.exp((1 - r) * W[10]) - 1);
...
export function recalcFsrsLocally(
  rating: SrsRating,
