...
```json
// Request
{ "email": "user@example.ru", "password": "min-8-chars" }

// Response 201
{
  "success": true,
  "data": {
    "user": { "id": "uuid", "email": "user@example.ru", "xp": 0, "currentStreak": 0 },
    "accessToken": "eyJ...",
    "expiresIn": 900
  }
}
...
> **Ограничение по домену (PLAN_Features_v0.3 §3).** Регистрация
> разрешена только с email в домене `.ru` (требование 152-ФЗ о
> локализации ПД на территории РФ). Список допустимых TLD
> конфигурируется на сервере переменной `ALLOWED_EMAIL_TLDS`
> (comma-separated, default `ru`). И клиент (`RegisterSchema`),
> и сервер (`auth.service.registerUser`) валидируют независимо.
...
### GET /api/stats/study-map

Карта изучения — прогресс пользователя по каждой колоде (PLAN_Features_v0.3 §5).
Возвращает `StudyMapResponseSchema` (`packages/shared/src/schemas/stats.ts`):
массив `DeckProgress` (`deckId`, `deckName`, `isSystemDeck`, `totalWords`,
`learnedWords`, `percentage` 0..100, `color`) + агрегаты `totalWords`,
`totalLearned`, `overallPercentage`.

«Изученным» считается слово в состоянии `graduated` (финальный уровень
SRS, согласовано с `getOverview.learnedWords`). Колоды отсортированы:
сначала системные (HSK), потом кастомные; внутри групп — по убыванию
процента изучения.
...
Вкладка «Тестирование» в web-клиенте. Генерирует тест из 20–30 случайных
слов выбранного HSK-уровня (1–6) с 6 разными типами вопросов:
выбор перевода, выбор иероглифа, набор пиньиня, тон на слух,
собрать слово из иероглифов, вставить пропущенное слово.

Активная сессия теста хранится в **Redis** (`test:session:<id>`, TTL 2ч)
и не персистится — после `submitTest` сессия удаляется, а финальный
    ...
    "questions": [
      {
        ...
        "wordCharacter": "喜欢",
        "wordPinyin": "xǐ huān",
        "wordTranslation": "нравиться, любить",
        "wordHskLevel": 2,
        "wordAudioUrl": null,
...
{
  "answers": [
    { "questionId": "q-uuid-1", "answer": "нравиться, любить" },
    { "questionId": "q-uuid-2", "answer": "xi3 huan1" }
  ],
  "timeSpentMs": 184000
}
...
// Response
{
  ...
  "data": {
    ...
    "timeSpentMs": 184000,
    "breakdown": [
      { "type": "multiple-choice-translation", "correct": 4, "total": 4 },
      { "type": "reverse-choice-character", "correct": 3, "total": 4 },
      { "type": "pinyin-input", "correct": 4, "total": 4 },
      { "type": "tone-recognition", "correct": 3, "total": 4 },
      { "type": "character-assembly", "correct": 2, "total": 4 },
      { "type": "cloze", "correct": 3, "total": 4 }
    ],
    "answers": [
      {
        "questionId": "q-uuid-1",
        "userAnswer": "нравиться, любить",
        "correctAnswer": "нравиться, любить",
        "isCorrect": true,
        "type": "multiple-choice-translation",
        "wordId": "w-uuid",
    ...
    "completedAt": "2026-07-07T12:03:04.000Z"
  }
}
```
...
## Чтение (Reading, PLAN_Features_v0.3 §7)

Все эндпоинты: `GET/POST /api/reading/*`
...
жадной токенизации по словарю `Word`. Каждый токен содержит
поверхность, позицию, ссылку на слово (`character`, `pinyin`,
`translation`), состояние изучения (`state`) и признак
`isPriority`.

### POST /api/reading/texts/:id/priority-words

Добавляет слова из текста в приоритетный список пользователя.
...
- **Схемы**: `RegisterSchema`, `LoginSchema`, `AuthResponseSchema`, `RefreshSchema` →
  `packages/shared/src/schemas/auth.ts`.
  - `RegisterSchema.email` дополнительно проверяет TLD через
    `isAllowedEmailTld` (PLAN_Features_v0.3 §3, default `ru`).
- **OAuth**: 3 провайдера на сервере (Google, Apple, Яндекс), но
  в web-клиенте активна только кнопка «Войти через Яндекс» —
  PLAN_Features_v0.3 §3 (сервис работает только с российскими
  провайдерами ради соответствия 152-ФЗ). Обмен через
