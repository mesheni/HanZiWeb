...
describe('recalcFsrs', () => {
  it('first-time "Easy" (rating 4) sets state to learning with 1-day interval', () => {
    const out = recalcFsrs(4, 0, 5, 'new');
  ...
  it('first-time "Again" (rating 1) keeps state in learning with 0-day interval', () => {
    const out = recalcFsrs(1, 0, 5, 'new');
    expect(out.newState).toBe('learning');
    expect(out.intervalDays).toBe(0);
  });

  it('"Again" demotes a graduated word back to review', () => {
    const out = recalcFsrs(1, 30, 3, 'graduated');
    expect(out.newState).toBe('review');
    expect(out.intervalDays).toBe(0);
  });

  it('"Easy" on a stable review card promotes to graduated when stability >= 21', () => {
    const out = recalcFsrs(4, 25, 2, 'review');
    expect(out.newState).toBe('graduated');
    expect(out.intervalDays).toBeGreaterThan(0);
  });

  it('clamps difficulty to the canonical FSRS-5 scale [1, 10] (PLAN_Features_v0.4 §46)', () => {
    const out1 = recalcFsrs(4, 5, 1, 'review');
    expect(out1.newDifficulty).toBeGreaterThanOrEqual(1);
    expect(out1.newDifficulty).toBeLessThanOrEqual(10);

    const out2 = recalcFsrs(1, 5, 10, 'review');
    expect(out2.newDifficulty).toBeGreaterThanOrEqual(1);
    expect(out2.newDifficulty).toBeLessThanOrEqual(10);
  });

  it('clamps stability to [0.01, 36500]', () => {
    const out = recalcFsrs(4, 1e9, 5, 'review');
